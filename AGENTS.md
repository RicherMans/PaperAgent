...
| `session/` | `Paper` and `Message` data models. Thread-safe `Manager` (mutex-protected) for CRUD + persistence to `~/.paperpaper/papers/{id}.json`. |
...
| `tui/` | Bubble Tea Elm architecture. `model.go` (state), `update.go` (commands & events), `view.go` (rendering). Three modes: Normal, Input, List. |
| `urlparse/` | `FetchURL()` tries external `arxiv2text` binary first, falls back to HTTP GET. `LoadFile()` reads with `~` expansion. |
...
`/new`, `/list`, `/open`, `/delete`, `/edit`, `/del`, `/summarize`, `/export`, `/model`, `/config`, `/help`, `/quit`
